...
const caseDetail: CaseSummary = {
  ...
  collaborationActions: [{ action: 'comment', name: 'Add comment', href: '/cases/case-1/comments', method: 'POST' }],
}
...
describe('case workspace', () => {
  it('loads a coherent workspace snapshot in parallel', async () => {
    ...
    const calls = installFetchScript((call) => {
      ...
      if (call.url.endsWith('/comments')) return { body: [{ id: 'comment-1', caseId: 'case-1', author: 'alice', text: 'Customer called', createdAt: '2026-09-01T09:00:00Z' }] }
    ...
    expect(screen.queryByText('Customer called')).not.toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Activity' }))
    expect(await screen.findByText('Customer called')).toBeInTheDocument()
